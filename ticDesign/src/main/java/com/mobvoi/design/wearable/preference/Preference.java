...
package com.mobvoi.design.wearable.preference;
...
import com.mobvoi.design.wearable.internal.CharSequences;
...
public class Preference implements Comparable<Preference> {
    ...
    private int mLayoutResId = R.layout.preference_ticwear;
     ...
     * {@link PreferenceViewHolder} needs to be create.
     *
    ...
    public Preference(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        for (int i = a.getIndexCount() - 1; i >= 0; i--) {
            ...
            } else if (attr == R.styleable.Preference_android_key) {
                mKey = a.getString(attr);

            } else if (attr == R.styleable.Preference_android_title) {
                mTitleRes = a.getResourceId(attr, 0);
                mTitle = a.getString(attr);

            } else if (attr == R.styleable.Preference_android_summary) {
                mSummary = a.getString(attr);

            } else if (attr == R.styleable.Preference_android_order) {
                mOrder = a.getInt(attr, mOrder);

            } else if (attr == R.styleable.Preference_android_fragment) {
            ...
            } else if (attr == R.styleable.Preference_android_enabled) {
                mEnabled = a.getBoolean(attr, true);

            } else if (attr == R.styleable.Preference_android_selectable) {
                mSelectable = a.getBoolean(attr, true);

            } else if (attr == R.styleable.Preference_android_persistent) {
                mPersistent = a.getBoolean(attr, mPersistent);

            } else if (attr == R.styleable.Preference_android_dependency) {
                mDependencyKey = a.getString(attr);

            } else if (attr == R.styleable.Preference_android_defaultValue) {
    ...
    static class ViewHolder extends PreferenceViewHolder {
        ...
        protected void bindPreferenceToData(@NonNull Preference preference) {
            ...
            data.summary = preference.getSummary();
            data.icon = getIconDrawable(preference);
        }

        private Drawable getIconDrawable(@NonNull Preference preference) {
            if (iconView != null && preference.mIconResId != 0 && preference.mIcon == null) {
                preference.mIcon = preference.getContext().getDrawable(preference.mIconResId);
            }
