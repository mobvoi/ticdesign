...
import com.mobvoi.design.wearable.widget.CoordinatorLayout.Behavior;
...
abstract class HeaderBehavior<V extends View> extends ViewOffsetBehavior<V> {
    ...
    public boolean onTouchEvent(CoordinatorLayout parent, V child, MotionEvent ev) {
        ...
        switch (MotionEventCompat.getActionMasked(ev)) {
            ...
            case MotionEvent.ACTION_UP:
                if (!needSnapToZero() && mVelocityTracker != null) {
                    mVelocityTracker.addMovement(ev);
                ...
                // $FALLTHROUGH
            case MotionEvent.ACTION_CANCEL: {
                snapToZeroOffsetIfNeeded(parent, child);
                mIsBeingDragged = false;
                mActivePointerId = INVALID_POINTER;
                if (mVelocityTracker != null) {
                    mVelocityTracker.recycle();
    ...
    final int scroll(CoordinatorLayout coordinatorLayout, V header,
        ...
        return setHeaderTopBottomOffset(coordinatorLayout, header,
                getTopBottomOffsetForScrollingSibling() + getOverScrollOffset() - dy,
                minOffset, maxOffset);
    }
    ...
    boolean needSnapToZero() {
        return getTopBottomOffsetForScrollingSibling() + getOverScrollOffset() > 0;
    }

    boolean snapToZeroOffsetIfNeeded(CoordinatorLayout coordinatorLayout, V child) {
        return needSnapToZero();
    }
